...
export class ApiService {
  ...
  constructor(private _http : HttpClient,private _router : Router,private navCtrl: NavController) { 
  ...
  userMobileEmailOtpVerify(formData : any){
...
userSupport(formData : any){
  ...
  /* Noted : userId For All Notification
    notification id for 1notification*/

  notificationAllClear(userId,status){
    return this._http.post<any>(_apiUrl+'notification/notification-clear',{
      cleared: status,
      notificationId: "",
      userId : userId,
  ...
  notificationMarkAsRead(userId,notificationId,status){
    let data = {
      notificationId : notificationId,
      userId : userId,
      status : status
    };
    return this._http.post<any>(_apiUrl+'notification/notification-status-change',data);
  }
